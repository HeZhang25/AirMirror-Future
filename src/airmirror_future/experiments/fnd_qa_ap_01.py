        ...
        for geometry_case in geometry_cases:
            ...
            for pattern_class, pattern_seed in pattern_descriptors:
                ...
                for rule, ox, oy in base_specs:
                    ...
                    row_meter.sample()
                    row = {"quadrature_rule": rule, "quadrature_order_x": ox, "quadrature_order_y": oy, **value, "coefficient_model_identity": controller_ris_coefficient_identity(scene, engine, scene.transmitter(), scene.receiver(), ris, quadrature_spec=spec, quadrature_policy_id=QUADRATURE_POLICY_ID, quadrature_policy_version=QUADRATURE_POLICY_VERSION), "quadrature_runtime_s": elapsed, "quadrature_peak_rss_mb": row_meter.finish()}
                ...
                try:
                    ...
                    for rule, ox, oy in (("midpoint", 32, 32), ("tensor_product_gauss_legendre", 32, 32)):
                        ...
                        elapsed = time.perf_counter() - t0
                        row_meter.sample()
                        row32 = {"quadrature_rule": rule, "quadrature_order_x": ox, "quadrature_order_y": oy, **value, "coefficient_model_identity": controller_ris_coefficient_identity(scene, engine, scene.transmitter(), scene.receiver(), ris, quadrature_spec=spec, quadrature_policy_id=QUADRATURE_POLICY_ID, quadrature_policy_version=QUADRATURE_POLICY_VERSION), "quadrature_runtime_s": elapsed, "quadrature_peak_rss_mb": row_meter.finish()}
                ...
                for raw_row in raw_rows:
                    if raw_row.get("series_identity") == series_identity:
                        matching = next(
                            ...
                            if candidate["quadrature_rule"] == raw_row["quadrature_rule"]
                            and candidate["quadrature_order_x"] == raw_row["quadrature_order_x"]
                            and candidate["quadrature_order_y"] == raw_row["quadrature_order_y"]
                        )
                        raw_row["coefficient_model_identity"] = matching[
