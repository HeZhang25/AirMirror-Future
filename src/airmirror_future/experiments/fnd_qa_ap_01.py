...
def evaluate_quadrature(
    ...
    *,
    ...
    h_ris = complex(np.dot(coefficients, gamma))
    baseline_result = active_engine.compute_channel(scene, ris_patterns={}, model=ControllerModel())
    h_baseline = complex(baseline_result.los_channel + baseline_result.wall_channel)
    return {"a": coefficients, "gamma": gamma, "h_ris": h_ris, "h_baseline": h_baseline, "h_total": h_baseline + h_ris}
...
def compare_to_reference(candidate: dict[str, Any], reference: dict[str, Any]) -> dict[str, Any]:
    ...
    h_ris_ref = reference["h_ris"]
    h_total_ref = reference["h_total"]
    gamma = reference["gamma"]
    s_ris = max(float(np.sum(np.abs(a_ref * gamma))), abs(h_ris_ref), AGGREGATE_FLOOR)
    s_total = max(abs(reference["h_baseline"]) + float(np.sum(np.abs(a_ref * gamma))), abs(h_total_ref), AGGREGATE_FLOOR)
    ris = _complex_metrics(candidate["h_ris"], h_ris_ref, s_ris)
    total = _complex_metrics(candidate["h_total"], h_total_ref, s_total)
    return {
        ...
        "candidate_deep_null_h_ris": abs(candidate["h_ris"]) / s_ris <= DEEP_NULL_RATIO,
        "reference_deep_null_h_total": abs(h_total_ref) / s_total <= DEEP_NULL_RATIO,
        "candidate_deep_null_h_total": abs(candidate["h_total"]) / s_total <= DEEP_NULL_RATIO,
...
def run(output: Path | None = None, *, generations: Iterable[str] = ("Current", "Advanced", "Future"), geometry_cases: Iterable[str] = tuple(GEOMETRY_CASES), include_random: bool = True) -> tuple[Path, Path, Path]:
    ...
    for generation in generations:
        for geometry_case in geometry_cases:
            ...
            patterns: list[tuple[str, int | None, np.ndarray, object]] = []
            p, f = _pattern_for_series(scene, focus_rx, geometry_case, "ris_only_focus", None, engine); patterns.append(("ris_only_focus", None, p, f))
            p, f = _pattern_for_series(scene, focus_rx, geometry_case, "coherent_target_focus", None, engine); patterns.append(("coherent_target_focus", None, p, f))
            if include_random:
                for seed in RANDOM_PATTERN_SEEDS:
                    p = deterministic_random_pattern(ris, generation, geometry_case, seed)
                    patterns.append(("random_legal", seed, p, generate_ris_only_focus_pattern))
            for pattern_class, pattern_seed, pattern, focus_callable in patterns:
                ...
                for rule, ox, oy in [("midpoint", x, y) for x, y in MIDPOINT_ORDERS] + [("tensor_product_gauss_legendre", 16, 16)]:
                    spec = midpoint_quadrature(ris, ox, oy) if rule == "midpoint" else tensor_product_gauss_legendre(ris, ox, oy)
                    t0 = time.perf_counter(); value = evaluate_quadrature(scene, pattern, spec, engine=engine); elapsed = time.perf_counter() - t0
                    row = {"quadrature_rule": rule, "quadrature_order_x": ox, "quadrature_order_y": oy, **value, "quadrature_runtime_s": elapsed}
                    if previous is not None:
                        diff = compare_to_reference(row, previous)
                        row["successive"] = diff
                ...
                try:
                    ...
                    # The frozen hierarchy permits 32x32 only after the base
                    # 16x16 convergence/cross-rule screen fails.  Evaluate the
                    # matched midpoint/GL pair and retry selection; unresolved
                    # cases remain explicit failures in the raw artifact.
                    for rule, ox, oy in (("midpoint", 32, 32), ("tensor_product_gauss_legendre", 32, 32)):
                        spec = midpoint_quadrature(ris, ox, oy) if rule == "midpoint" else tensor_product_gauss_legendre(ris, ox, oy)
                        t0 = time.perf_counter(); value = evaluate_quadrature(scene, pattern, spec, engine=engine); elapsed = time.perf_counter() - t0
                        row32 = {"quadrature_rule": rule, "quadrature_order_x": ox, "quadrature_order_y": oy, **value, "quadrature_runtime_s": elapsed}
                    ...
                    try:
                        ...
                        reference = rows[-1]; reference_label = "unresolved_reference"; status_reference = "fail"
                ref_id = f"{generation}:{geometry_case}:{pattern_class}:{pattern_seed}"
    ...
    artifact_bytes = json.dumps(artifact_records, ensure_ascii=False, sort_keys=True, separators=(",", ":")).encode("utf-8")
    ...
    # The selected coefficient artifact is immutable and linked by identity in
    # both raw and summary outputs.  Keep the complete vector outside ordinary
    # CSV cells while retaining the frozen parent ordering.
    for row in raw_rows:
        row["a_reference_artifact_identity"] = artifact_identity
        row["run_runtime_s"] = time.perf_counter() - started_run
    ...
    # Keep the summary deterministic and keyed to the declared candidate rows;
    # detailed per-order vectors remain in raw CSV and the coefficient artifact.
    for record in summary_records:
        record["series_runtime_s"] = series_runtime_by_identity.get(str(record["series_identity"]), "")
        record["series_peak_rss_mb"] = ""
        record["run_runtime_s"] = run_elapsed
        record["run_peak_rss_mb"] = ""
    ...
    run_path.write_text(json.dumps({"qa_schema_id": QA_SCHEMA_ID, "qa_schema_version": QA_SCHEMA_VERSION, "run_id": run_id, "run_runtime_s": time.perf_counter() - started_run, "peak_memory_method": "unavailable", "python": platform.python_version(), "os": platform.platform(), "blas_environment": {key: os.environ.get(key, "") for key in ("OMP_NUM_THREADS", "MKL_NUM_THREADS", "OPENBLAS_NUM_THREADS", "NUMEXPR_NUM_THREADS", "VECLIB_MAXIMUM_THREADS", "BLIS_NUM_THREADS", "MKL_DYNAMIC")}, "base_minimum_matrix_wall_budget_h": 8}), encoding="utf-8")
