...
vertical slice 与现有桌面前端中的 cached field result-playback Demo 可运行，不是 release
capability 的 Completed/Verified。
...
- 同一 production solve 产生的 cached Static RIS / No RIS spatial field visualization。
...
运行满足。GUI 依次后台预计算一次 11×2 link states 与一次 Static RIS Fast `80×60` field map；
No RIS power 复用该结果的 baseline，No RIS SNR 按同一 noise semantics 派生。Play/Pause/Reset、
slider、mode/quantity 切换均不重算 channel 或 field-map physics。
