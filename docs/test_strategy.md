...
| FND-T13d | reflecting-wall exclusion | 反射墙不作为自身路径 blocker；其他阻挡仍只作用于命中的路径段 | `test_reflecting_wall_is_excluded_from_reflection_leg_blockers`、`test_mutated_duplicate_wall_ids_fail_before_profile_or_world`、constructor/loader test；C1 Implemented |
...
self-exclusion 排除多堵墙。Context 不新增通用 minimum-distance 拒绝；reflection 总路径与 RIS
