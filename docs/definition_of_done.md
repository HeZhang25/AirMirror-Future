...
7. C2 新实验使用稳定 schema/version 和 exclusive no-overwrite run directory；v0.1 legacy 与 A/B
   ...
   FND-PHY-NB 与 FND-QA-AP 已签署的 frequency/quadrature production identity 必须由唯一代码
   来源记录；尚未由 FND-QA-CC 签署的 coefficient identity 只能为空/显式 candidate。新结果仍因
   FND-QA-CC 保持 `partial`/pending，不得伪造 complete/Verified provenance；
