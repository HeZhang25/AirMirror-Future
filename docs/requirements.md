...
|---|---|
| 文档状态 | Normative |
| 基线版本 | v0.1 + Foundation A1-A3/FND-FIX-WALL/B Verified + C1/C2 Verified |
...
| AMF-RIS-008 | 区分 RIS-only 与 Coherent Target Focus；GUI 默认算法与 nominal target objective 一致 | Verified | `ris/phase.py`, `optimization/coherent_focus.py`, `gui/main_window.py` | A1 `tests/test_coherent_focus.py` FND-T01..05；B `test_gui_defaults_to_coherent_target_and_keeps_ris_only_accessible`；真实 GUI 人工验收与 A/B checkpoint 外部独立审查 PASS、blocking issues 0；ADR-0006；[B Work Item](work_items/foundation_0_1_1_b.md) |
| AMF-RIS-009 | `nx/ny` 定义为 equivalent controllable aperture patches，并显示有效 pitch/波长比例和限制 | Verified | `ris/aperture.py`, `gui/main_window.py`, ADR-0007 | A2 `tests/test_aperture_diagnostics.py` FND-T09；B `test_pattern_metadata_and_ground_truth_labels_are_explicit` 的 pitch/λ metadata；真实 GUI 人工验收与 A/B checkpoint 外部独立审查 PASS、blocking issues 0；[A2 Work Item](work_items/foundation_0_1_1_a2.md)、[B Work Item](work_items/foundation_0_1_1_b.md) |
...
| AMF-RIS-011 | 在 Foundation final exit/P1A 前固定 aperture/control/pattern、仅细化独立 quadrature，冻结可重放的 coefficient policy 和声明适用域 | Verified | [ADR-0008](adr/0008-minimum-aperture-quadrature-validity-gate.md)、[FND-QA-AP](work_items/foundation_0_1_1_qa_ap.md) | FND-QA-AP v1 formal run `20260906T094526-de4745c3` 与 reference-resolution continuation `20260906T123708-78615a33`；84/84 series resolved，signed/frozen midpoint `8×8` policy 已接入 production RIS scattering；control/pattern semantics 不变；不等同 EM/full-wave truth |
...
| AMF-UI-007 | 参数编辑具有 pending/apply/Optimize 门禁；preset 覆盖和 Customized 状态可辨认 | Verified | `gui/main_window.py` | `tests/test_gui_smoke.py::test_pending_parameters_block_optimize_and_preserve_applied_scene`, `test_generation_cancel_preserves_pending_controls`, `test_generation_confirm_discards_pending_and_marks_customized_display`, `test_generation_customized_is_display_only`；真实 GUI 人工验收；B2/A/B checkpoint 外部独立审查 PASS、blocking issues 0；[B Work Item](work_items/foundation_0_1_1_b.md) |
| AMF-UI-008 | Pattern 显示 grid/bits/states/source/error/legend；Ground Truth 标签与真实作用范围一致 | Verified | `gui/pattern_view.py`, `gui/main_window.py` | `tests/test_gui_smoke.py::test_pattern_metadata_and_ground_truth_labels_are_explicit`, `test_continuous_pattern_does_not_infer_hardware_states_from_search_levels`, `test_ris_gain_heatmap_uses_zero_centered_diverging_normalization`；真实 GUI 人工验收与 B3 visualization review PASS、blocking issues 0；[B Work Item](work_items/foundation_0_1_1_b.md) |
...
| AMF-EXP-006 | 实验分开记录 focus mode、profile/reflection/channel model version、search levels，且不覆盖 legacy 结果 | Verified | [C Work Item](work_items/foundation_0_1_1_c.md)、[experiment_spec.md](experiment_spec.md) | C2 independent review PASS（reviewed SHA `a5c434dd3d673069ea93c689e6435002aef4e83d`）；`tests/test_phase_bits_integration.py`、完整 pytest；Current/Advanced/Future fast headless；真实 C2 run、FND-T15..T15d |
